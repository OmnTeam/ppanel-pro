...
func (uc *ticketUseCase) CreateTicket(ctx context.Context, params *CreateTicketParams) error {
	uc.logger.Infof("[CreateTicket] tenantID: %d, userID: %d, title: %s", params.TenantID, params.UserID, params.Title)

	// 调用repo创建工单
	err := uc.repo.CreateTicket(ctx, params.TenantID, params.UserID, params.Title, params.Description)
	if err != nil {
...
func (uc *ticketUseCase) GetTicketList(ctx context.Context, params *GetTicketListParams) (*GetTicketListResult, error) {
	uc.logger.Infof("[GetTicketList] tenantID: %d, userID: %d, page: %d, size: %d",
		params.TenantID, params.UserID, params.Page, params.Size)

	total, list, err := uc.repo.GetTicketList(ctx, params.TenantID, params.UserID,
		params.Page, params.Size, params.Status, params.Search)
	if err != nil {
...
func (uc *ticketUseCase) GetTicketDetails(ctx context.Context, params *GetTicketDetailsParams) (*TicketInfo, error) {
	uc.logger.Infof("[GetTicketDetails] tenantID: %d, userID: %d, ticketID: %d",
		params.TenantID, params.UserID, params.ID)

	// 查询工单基本信息
	ticket, err := uc.repo.GetTicketByID(ctx, params.TenantID, params.ID)
	if err != nil {
	...
	// 查询工单跟进记录
	follows, err := uc.repo.GetTicketFollows(ctx, params.TenantID, params.ID)
	if err != nil {
...
func (uc *ticketUseCase) UpdateTicketStatus(ctx context.Context, params *UpdateTicketStatusParams) error {
	uc.logger.Infof("[UpdateTicketStatus] tenantID: %d, userID: %d, ticketID: %d, status: %d",
		params.TenantID, params.UserID, params.ID, params.Status)

	// 调用repo更新状态（repo层会验证用户权限）
	err := uc.repo.UpdateTicketStatus(ctx, params.TenantID, params.UserID, params.ID, params.Status)
	if err != nil {
...
func (uc *ticketUseCase) CreateTicketFollow(ctx context.Context, params *CreateTicketFollowParams) error {
	uc.logger.Infof("[CreateTicketFollow] tenantID: %d, userID: %d, ticketID: %d",
		params.TenantID, params.UserID, params.TicketID)

	// 1. 查询工单，验证权限
	ticket, err := uc.repo.GetTicketByID(ctx, params.TenantID, params.TicketID)
	if err != nil {
	...
	// 2. 创建跟进记录
	err = uc.repo.CreateTicketFollow(ctx, params.TenantID, params.TicketID,
	...
	// 3. 将工单状态更新为Pending（待处理）
	err = uc.repo.UpdateTicketStatus(ctx, params.TenantID, params.UserID, params.TicketID, StatusPending)
	if err != nil {
