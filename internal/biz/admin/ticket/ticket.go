...
type TicketRepo interface {
	...
	GetTicketList(ctx context.Context, page, size int, userId int64, status *int8, search string) (int64, []*Ticket, error)
	// UpdateTicketStatus 更新工单状态
	UpdateTicketStatus(ctx context.Context, id int64, status int8) error
...
// GetTicket 获取工单详情
func (uc *TicketUseCase) GetTicket(ctx context.Context, id int64) (*Ticket, error) {
...
// GetTicketList 获取工单列表
func (uc *TicketUseCase) GetTicketList(ctx context.Context, page, size int, userId int64, status *int8, search string) (int64, []*Ticket, error) {
...
// UpdateTicketStatus 更新工单状态
func (uc *TicketUseCase) UpdateTicketStatus(ctx context.Context, id int64, status int8) error {
	// 检查工单是否存在
	_, err := uc.repo.GetTicketById(ctx, id)
	if err != nil {
		uc.log.Errorf("UpdateTicketStatus GetTicketById error: %v, id=%d", err, id)
		return err
	}
...
func (uc *TicketUseCase) CreateTicketFollow(ctx context.Context, follow *Follow) error {
	// 检查工单是否存在
	_, err := uc.repo.GetTicketById(ctx, follow.TicketId)
	if err != nil {
		uc.log.Errorf("CreateTicketFollow GetTicketById error: %v, ticketId=%d", err, follow.TicketId)
		return err
	}
	...
	// 更新工单状态为 Waiting (2)
	err = uc.repo.UpdateTicketStatus(ctx, follow.TicketId, 2)
	if err != nil {
		uc.log.Errorf("CreateTicketFollow UpdateTicketStatus error: %v, ticketId=%d", err, follow.TicketId)
		return err
	}
