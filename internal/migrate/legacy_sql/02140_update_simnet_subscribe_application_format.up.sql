...
SET
  `description` = 'OmnXT SimNet base64 subscription',
  `subscribe_template` = '{{- range $link := buildOmnxtProtocolLinks .Proxies .UserInfo .Params }}{{ $link }}
{{- end }}',
  `output_format` = 'base64',
  `updated_at` = NOW(3)
WHERE `name` = 'OmnXT SimNet';

UPDATE `subscribe_application`
SET
  `subscribe_template` = '{{- $GiB := 1073741824.0 -}}
...
{{- range $link := buildOmnxtProtocolLinks .Proxies .UserInfo .Params }}
{{ $link }}
{{- end }}',
  `updated_at` = NOW(3)
WHERE `name` = 'Default' AND `output_format` = 'base64';
