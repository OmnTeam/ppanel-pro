...
func addNode(data *Server, host string, port int) *proxy.Proxy {
	var option any
...
func adapterRules(groups []*RuleGroup) (proxyGroup []proxy.Group, rules []string, defaultGroup string) {
...
func adapterProxies(servers []*Server) ([]proxy.Proxy, []string, map[string][]string) {
	var proxies []proxy.Proxy
	var tags = make(map[string][]string)
	for _, node := range servers {
		switch node.RelayMode {
		case RelayModeAll:
			var relays []NodeRelay
