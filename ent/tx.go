...
func (tx *Tx) init() {
	tx.ProxyAds = NewProxyAdsClient(tx.config)
	tx.ProxyAnnouncement = NewProxyAnnouncementClient(tx.config)
	...
	tx.ProxySubscribe = NewProxySubscribeClient(tx.config)
	...
	tx.ProxyTicket = NewProxyTicketClient(tx.config)
	tx.ProxyTicketFollow = NewProxyTicketFollowClient(tx.config)
	tx.ProxyTrafficLog = NewProxyTrafficLogClient(tx.config)
	tx.ProxyUser = NewProxyUserClient(tx.config)
...
// applies a query, for example: ProxyAds.QueryXXX(), the query will be executed
