...
func (ProxySubscribe) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Int64("inventory").Default(-1).Comment("库存"),
		field.Int64("traffic").Default(0).Comment("流量"),
		field.Int64("speed_limit").Default(0).Comment("速度限制"),
		field.Int64("device_limit").Default(0).Comment("设备数限制"),
		field.Int64("quota").Default(0).Comment("配额"),
		...
		field.Int64("deduction_ratio").Optional().Nillable().Default(0).Comment("扣除比例"),
		field.Bool("allow_deduction").Default(true).Comment("允许扣除"),
		field.Int64("reset_cycle").Optional().Nillable().Default(0).Comment("重置周期"),
