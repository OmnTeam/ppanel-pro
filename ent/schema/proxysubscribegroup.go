...
func (ProxySubscribeGroup) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Int64("expired_days_limit").Optional().Nillable().Default(0).Comment("过期天数限制"),
		field.Int64("max_traffic_gb_expired").Optional().Nillable().Default(0).Comment("过期组最大流量GB"),
