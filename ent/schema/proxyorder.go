package schema

import (
	"time"

	"entgo.io/ent"
	"entgo.io/ent/dialect/entsql"
	"entgo.io/ent/schema"
	"entgo.io/ent/schema/field"
)

// ProxyOrder holds the schema definition for the ProxyOrder entity
// 订单管理表
type ProxyOrder struct {
	ent.Schema
}

// Annotations of the ProxyOrder
func (ProxyOrder) Annotations() []schema.Annotation {
	return []schema.Annotation{
		entsql.Annotation{Table: "proxy_order"},
		entsql.WithComments(true),
	}
}

// Fields of the ProxyOrder
func (ProxyOrder) Fields() []ent.Field {
	return []ent.Field{
		field.Int64("id").
			Comment("订单ID"),
		field.Int64("parent_id").
			Optional().
			Comment("父订单ID"),
		field.Int64("user_id").
			Default(0).
			Comment("用户ID"),
		field.String("order_no").
			NotEmpty().
			Unique().
			Comment("订单号"),
		field.Int8("type").
			Default(1).
			Comment("订单类型：1:订阅 2:续费 3:重置流量 4:充值"),
		field.Int64("quantity").
			Default(1).
			Comment("数量"),
		field.Int64("price").
			Default(0).
			Comment("原价"),
		field.Int64("amount").
			Default(0).
			Comment("实付金额"),
		field.Int64("discount").
			Default(0).
			Comment("折扣金额"),
		field.String("coupon").
			Optional().
			MaxLen(255).
			Comment("优惠券代码"),
		field.Int64("coupon_discount").
			Default(0).
			Comment("优惠券折扣金额"),
		field.Int64("commission").
			Default(0).
			Comment("佣金金额"),
		field.Int64("fee_amount").
			Default(0).
			Comment("手续费金额"),
		field.Int64("gift_amount").
			Default(0).
			Comment("赠送金额"),
		field.Int64("payment_id").
			Default(0).
			Comment("支付方式ID"),
		field.String("method").
			Optional().
			MaxLen(255).
			Comment("支付方式标识"),
		field.String("trade_no").
			Optional().
			MaxLen(255).
			Comment("第三方交易号"),
		field.Int8("status").
			Default(0).
			Comment("订单状态：0待支付 1已支付 2已取消 3已退款"),
		field.Int64("subscribe_id").
			Default(0).
			Comment("关联订阅ID"),
		field.String("subscribe_token").
			Optional().
			MaxLen(255).
			Comment("续费订阅Token"),
		field.Bool("is_new").
			Default(false).
			Comment("是否新订单"),
		field.Time("created_at").
			Default(time.Now).
			Immutable().
			Comment("创建时间"),
		field.Time("updated_at").
			Default(time.Now).
			UpdateDefault(time.Now).
			Comment("更新时间"),
	}
}

// Edges of the ProxyOrder
func (ProxyOrder) Edges() []ent.Edge {
	return []ent.Edge{}
}

// Indexes of the ProxyOrder
func (ProxyOrder) Indexes() []ent.Index {
	return []ent.Index{}
}